  ...
  const handleLogin = async (e) => {
    ...
    if (step === 1) {
      ...
      try {
        ...
        if (data.code && data.verifyPhrase) {
          setCode(data.code);
          setVerifyPhrase(data.verifyPhrase);
          setCopiedPhrase("");
          setStep(2);
        } else {
      ...
      if (copiedPhrase !== verifyPhrase) {
        toast.error("Phrase doesn't match! Copy it correctly.");
        return;
      }
      setLoading(true);
      try {
        const response = await fetch(`${api}/login`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ username, code, verifyPhrase }),
        });
        const data = await response.json();
        if (data.hash) {
          localStorage.setItem("hxskpet", data.hash);
  ...
  return (
    <div className={loginstyles.blurbg} onClick={closeModal}>
      ...
      >
        ...
        <h2 className={loginstyles.title}>Welcome to hxskpet</h2>
            ...
            <>
              <div className={loginstyles.inputGroup}>
                <label className={loginstyles.label}>
                  Copy this phrase to verify
                </label>
                <div className={loginstyles.inputHolder}>
                  <input type="text" value={verifyPhrase} disabled={true} />
                  <FiCopy
                    className={loginstyles.copyIcon}
                    onClick={() => {
                      navigator.clipboard.writeText(verifyPhrase);
              ...
              <div className={loginstyles.inputGroup}>
                <label className={loginstyles.label}>
                  Paste the phrase here
                </label>
                <div className={loginstyles.inputHolder}>
                  <input
                    type="text"
                    placeholder="Paste the phrase you copied..."
                    value={copiedPhrase}
                    onChange={(e) => setCopiedPhrase(e.target.value)}
                  />
                </div>
              </div>
            </>
