...
const rollwinner = async (giveawayid) => {
    ...
    try {
        ...
        const res = await sendwebhook(giveawaywebh, "hxskpet Giveaway Concluded!", `A new **${giveaway.item[0].itemname}** giveaway has been concluded!`, [
            {
                name: "Host",
...
exports.getgiveaways = asyncHandler(async (req, res) => {
    try {
        ...
        console.warn("Giveaways fetch error (database unavailable):", err.message);
        // Return empty array when database is unavailable
        return res.status(200).json({ message: "OK", giveaways: [] });
    }
});
    ...
    try {
      await session.withTransaction(async () => {
        ...
        giveawaysToSave.forEach((giveaway) => {
          ...
          sendwebhook(
            giveawaywebh,
            "hxskpet Giveaway Created",
            `A new **${giveaway.item[0].itemname}** giveaway has been created in hxskpet. Join now at https://hxskpet.com/`,
            [
              {
                name: "Host",
                value: `\`\`${user.username}\`\``,
