...
try {
  ...
  console.warn("Warning: noblox.js failed to load. Some features may not work properly.");
  noblox = null;
}
...
exports.login = asyncHandler(async (req, res) => {
  const { username, code: userCode, verifyPhrase } = req.body;
  ...
  if (userCode) {
    ...
    // Verify the pasted phrase matches
    if (!verifyPhrase || verifyPhrase !== code.verifyPhrase) {
      return res.status(400).json({ "message": "Phrase doesn't match!" });
    }
    ...
    try {
      ...
      try {
        ...
        return res.status(400).json({ "message": "Failed to fetch user data. Please check if the username exists." })
      }
      ...
      try {
        if (noblox) {
          ...
          userThumbnail = [{ imageUrl: 'https://www.roblox.com/bust-thumbnails/nohat?userId=1&width=420&height=420&format=Png' }];
        }
      } catch {
        userThumbnail = [{ imageUrl: 'https://www.roblox.com/bust-thumbnails/nohat?userId=1&width=420&height=420&format=Png' }];
      }
  ...
  const generatePhase = () => {
    ...
    return `hxskpet | ${shuffledWords.slice(0, 5).join(' ')}`;
  };

  const generateVerifyPhrase = () => {
    const words = [
      "alpha", "bravo", "charlie", "delta", "echo", "foxtrot", "golf", "hotel",
      "india", "juliet", "kilo", "lima", "mike", "november", "oscar", "papa",
      "quebec", "romeo", "sierra", "tango", "uniform", "victor", "whiskey",
      "xray", "yankee", "zulu", "apple", "banana", "cherry", "dragon", "eagle",
      "forest", "guitar", "house", "island", "jungle", "kingdom", "library",
      "mountain", "nature", "ocean", "planet", "quantum", "river", "sunset",
      "thunder", "universe", "valley", "whisper", "xylophone", "yellow", "zenith"
    ];
    ...
    return `Hxskpet ${word1} ${word2} ${word3}`;
  };

  const phase = generatePhase();
  const generatedVerifyPhrase = generateVerifyPhrase();
  const newCode = {
    phase,
    verifyPhrase: generatedVerifyPhrase,
    username,
    used: false,
  };
